...
public class MyLinkedList<T> implements Iterable<T> {
    ...
    private class ListIter extends Iter implements ListIterator<T> {
        ...
        public void remove() {
            if (pastNode != null) {
                if (pastNode.getPrev() != null) { //если не первый
                    pastNode.getPrev().setNext(pastNode.getNext());
                }
                if (pastNode.getNext() != null) { //если не последний
                    pastNode.getNext().setPrev(pastNode.getPrev());
                }
                pastNode = null;
                size--;
            }
        ...
        //удаляет элемент у который прошли методом next или prev
        @Override
        public void set(T t) {
            if (pastNode != null) {
                pastNode.setValue(t);
        ...
        public void add(T t) {
            if (pastNode != null) {
                if (current.getPrev() == pastNode) {
                    insert(previousIndex(), t);
                } else {
                    insert(nextIndex(), t);
                }
                pastNode = null;
